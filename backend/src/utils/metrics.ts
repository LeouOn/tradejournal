...
export interface DBTrade {
  ...
  symbol: string;
  status: string;
  net_pnl: Decimal | number;
  r_multiple: Decimal | number;
  duration: number;
...
export interface MetricSummary {
  ...
  equityCurve: { tradeIndex: number; pnl: number; balance: number; date: string }[];
}

export function calculateMetrics(trades: DBTrade[], initialBalance: number): MetricSummary {
  ...
  let winRateScore = 0;
  if (winRate >= 0.5) winRateScore = 100;
  else if (winRate >= 0.3) winRateScore = ((winRate - 0.3) / 0.2) * 100;

  // Profit Factor Score: >= 1.5 (100 pts), 1.0 to 1.5 scaled, < 1.0 (0 pts)
  let pfScore = 0;
  if (profitFactor >= 1.5) pfScore = 100;
  else if (profitFactor >= 1.0) pfScore = ((profitFactor - 1.0) / 0.5) * 100;

  // Rule Adherence Score: >= 90% (100 pts), scaled down to 50%
  let ruleScore = 0;
  if (ruleAdherenceRate >= 0.9) ruleScore = 100;
  else if (ruleAdherenceRate >= 0.5) ruleScore = ((ruleAdherenceRate - 0.5) / 0.4) * 100;

  // Max Drawdown Score: <= 5% (100 pts), 5% to 15% scaled down, > 15% (0 pts)
  let ddScore = 0;
  if (maxDrawdownPercent <= 5) ddScore = 100;
  else if (maxDrawdownPercent <= 15) ddScore = ((15 - maxDrawdownPercent) / 10) * 100;

  // Recovery Factor Score: >= 3 (100 pts), scaled down to 0
  let rfScore = 0;
  if (recoveryFactor >= 3) rfScore = 100;
  else if (recoveryFactor > 0) rfScore = (recoveryFactor / 3) * 100;
