import { getSymbolMultiplier, getSymbolTickSize, validatePriceProximity } from "../utils/multipliers";
import { calculateMetrics, DBTrade } from "../utils/metrics";

describe("Futures Spec Utility Tests", () => {
  test("getSymbolMultiplier returns correct futures contract point values", () => {
    expect(getSymbolMultiplier("ES")).toBe(50);
    expect(getSymbolMultiplier("ESM6")).toBe(50);
    expect(getSymbolMultiplier("MES")).toBe(5);
    expect(getSymbolMultiplier("NQ")).toBe(20);
    expect(getSymbolMultiplier("MNQ")).toBe(2);
    expect(getSymbolMultiplier("MNQ2026")).toBe(2);
    expect(getSymbolMultiplier("RTY")).toBe(50);
    expect(getSymbolMultiplier("M2K")).toBe(5);
    expect(getSymbolMultiplier("YM")).toBe(5);
    expect(getSymbolMultiplier("MYM")).toBe(0.5);
    expect(getSymbolMultiplier("AAPL")).toBe(1); // Standard stock
  });

  test("getSymbolTickSize returns correct tick sizes", () => {
    expect(getSymbolTickSize("ES")).toBe(0.25);
    expect(getSymbolTickSize("MES")).toBe(0.25);
    expect(getSymbolTickSize("RTY")).toBe(0.1);
    expect(getSymbolTickSize("YM")).toBe(1.0);
    expect(getSymbolTickSize("TSLA")).toBe(0.01);
  });

  test("validatePriceProximity correctly flags price anomalies", () => {
    // 29638 vs 29608 (0.1% delta) - Sane
    expect(validatePriceProximity(29638, 29608).isValid).toBe(true);
    // 29638 vs 299600 (910% delta) - Anomalous
    expect(validatePriceProximity(29638, 299600).isValid).toBe(false);
    // 100 vs 131 (31% delta) - Anomalous (>30%)
    expect(validatePriceProximity(100, 131).isValid).toBe(false);
    // 100 vs 129 (29% delta) - Sane (<30%)
    expect(validatePriceProximity(100, 129).isValid).toBe(true);
  });
});
...
describe("Metrics Calculation Tests", () => {
  ...
  test("calculateMetrics calculates win rate, profit factor, drawdown, and rule adherence", () => {
    ...
    const metrics = calculateMetrics(mockTrades, initialBalance);
